...
class AlertaJonathasSupabaseUser extends BaseAuthUser {
  AlertaJonathasSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> alertaJonathasSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AlertaJonathasSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
